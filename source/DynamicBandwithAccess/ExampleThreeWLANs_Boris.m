function  [S,p]=ExampleThreeWLANs()
    ...
    % [SERGIO] Packet departure rate when using 1, 2, 4 or 8 channels? B:
    % yes
    mu(1)=1/(Ts1);
    mu(2)=1/(Ts2);
    mu(3)=1/(Ts3);
    ...
    n_chan = 8;% optional it can be omitted (default is 19) IT CAN SPEED UP COMPUTATION IS CHANNELS ARE NOT USED
    ...
    wlan(3).range = [1 2 3 4];
     ...
     for i=1:numel(wlan)
         if (max(wlan(i).range) - min(wlan(i).range))>n_chan
             ...
             %if any(wlan(i).primary == wlan(i).range)
    ...
    % [SERGIO] ???
    onlymax = false; % ALEX false to allow also intermediate widths
