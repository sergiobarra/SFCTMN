...
%% Hardocded transition parameters (alfa and beta) for equiprobable case
alfa_A1 = 0.5;
alfa_B1 = 0.5;
...
%% Optimal identification of alfa and beta

trans_rates_parameters = [0; 0];    % alfa and beta
optimal_S = 0;
k = 100;
J = zeros(k+1,k+1);
kA = 0;
for alfa_A1 = 0:1/k:1
    ...
    for alfa_B1 = 0:1/k:1
        ...
        if sum(S) > sum(optimal_S)
            trans_rates_parameters = [alfa_A1; alfa_B1];
            optimal_S = S;
            optimal_Q = Q;
            optimal_p = p;
        end
...
if show_optimal
    ...
    % disp('TRANSITION RATE MATRIX Q [1/s]')
    % disp(Q)
    disp(['alfa_A1 = ' num2str(trans_rates_parameters(1)) ', alfa_B1 = ' num2str(trans_rates_parameters(2))])
    disp(['Equilibrium distribution (prob. of being in each possible state) ---> Check: Sums ' num2str(sum(p))]);
    disp(p)
    disp('Average throughput per AP [Mbps]:');
    disp(optimal_S)
    disp('Average total throughput [Mbps]:');
